...
router.get('/', keycloak.enforcer({response_mode: 'token'}), async (req, res) => {
  var services = [
    ...
    {
      enabled: config.enabled.jenkins,
      'name': 'Jenkins',
    ...
    {
      'enabled': config.enabled.mattermost,
      'name': 'Mattermost',
    ...
    {
      'enabled': config.enabled.openproject,
      'name': 'OpenProject',
    ...
    {
      'enabled': config.enabled.keycloak,
      'name': 'Keycloak',
